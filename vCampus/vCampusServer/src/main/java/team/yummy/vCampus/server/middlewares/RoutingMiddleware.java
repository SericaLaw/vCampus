...
public class RoutingMiddleware implements Middleware {
    ...
    public void run(WebContext ctx, WebContext.MiddlewareInvoker next) {
        try {
            // 准备数据库
            URI db_uri = Server.class.getClassLoader().getResource("trial1.accdb").toURI();
            ...
            switch (ctx.request.getType()) {
                ...
                case POST: {
                    switch (ctx.request.getRoute()) {
                        case "/account/login":
                            ...
                            break;
                        case "/account":
                            boolean insertSuc = dbhelper.insert("Account", ctx.request.getJsonData());
                            if (insertSuc)  // 创建成功，201
                                ctx.response = new WebResponse("201", null, "OK");
                            else // 创建失败，403，往往是因为已经有创建过的了
                                ctx.response = new WebResponse("403", null, "User already created.");
                            break;
                ...
                case DELETE: {
                    ...
                    if (deleteSuc)
                        ...
                        ctx.response = new WebResponse("404", null, "Item not found.");
                }
                default:
